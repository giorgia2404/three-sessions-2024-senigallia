...
export function sketch() {
   ...
   for (let i = 0; i < p.numfireflies; i++) {
       ...
       const fireFly = new THREE.Mesh(fireFlyGeom, fireFlyMate);
       const fireFlyLight = new THREE.PointLight(0xFFC702, 2, 1);
       fireFlyLight.castShadow = true; // Abilita la creazione di ombre
       fireFlies.push(fireFly);
       fireFliesLights.push(fireFlyLight);
       scene.add(fireFlyLight);
       scene.add(fireFly)
   }
    ...
    const animate = () => {
        ...
        if (!paused) {
            ...
            for (let i = 0; i < p.numfireflies; i++) {
                fireFlies[i].position.x = -1 + noise3D(0, t2 + i, 0) * 2;
                fireFlies[i].position.y = 2 + noise3D(t2 + i + 4, 0, 0) * 4;
                fireFlies[i].position.z = -1 + noise3D(0, 0, t2 - i + 8) * 2;
                fireFliesLights[i].position.copy(fireFlies[i].position);
            
                // Modula l'intensità della luce in base al tempo e all'indice della lucciola
                const pulsationOffset = i * Math.PI / p.numfireflies; // Offset di fase per ogni lucciola
                const pulsationIntensity = Math.sin(t * pulsationFrequency * 2 * Math.PI + pulsationOffset) * 0.5 + 0.5;
                fireFliesLights[i].intensity = pulsationIntensity * 3; // Regola l'intensità in base alla pulsazione

                fireFlies[i].material.opacity = pulsationIntensity;
            }
        }
...
export function dispose() {
    ...
    fireFliesLights.forEach((fireLight) => {
        scene.remove(fireLight);
    });
    fireFlies = []
    fireFlies.length = 0;
    fireFliesLights = []
    fireFliesLights.length = 0;
    fireFlyGeom?.dispose();
    light?.dispose();
    lightD?.dispose();
