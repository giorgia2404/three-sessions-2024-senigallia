...
export function sketch() {
    ...
    const anchorDistance = 3;
    const anchorPoints = [
        new CANNON.Vec3(-cWidth / 2, p.floor + anchorDistance -1, -cHeight / 2), // top left
        new CANNON.Vec3(cWidth / 2, p.floor + anchorDistance -1, -cHeight / 2),  // top right
        new CANNON.Vec3(cWidth / 2, p.floor + anchorDistance -1 , cHeight / 2),   // bottom right
        new CANNON.Vec3(-cWidth / 2, p.floor + anchorDistance -1, cHeight / 2),  // bottom left
        new CANNON.Vec3(0, p.floor + anchorDistance, -cHeight / 2),           // top center
        new CANNON.Vec3(cWidth / 2, p.floor + anchorDistance, 0),             // right center
        new CANNON.Vec3(0, p.floor + anchorDistance, cHeight / 2),            // bottom center
        new CANNON.Vec3(-cWidth / 2, p.floor + anchorDistance, 0),            // left center
        new CANNON.Vec3(0, p.floor + anchorDistance + 10, 0),                  // center, slightly higher
    ];
    ...
    const anchorParticles = [
        clothParticles[0][0],             // top left
        clothParticles[Nx][0],            // top right
        clothParticles[Nx][Ny],           // bottom right
        clothParticles[0][Ny],            // bottom left
        clothParticles[Math.floor(Nx / 2)][0],        // top center
        clothParticles[Nx][Math.floor(Ny / 2)],       // right center
        clothParticles[Math.floor(Nx / 2)][Ny],       // bottom center
        clothParticles[0][Math.floor(Ny / 2)],        // left center
       // clothParticles[Math.floor(Nx / 2)][Math.floor(Ny / 2)], // center
    ];

    anchorParticles.forEach((particle, index) => {
    ...
    // Funzione per aggiornare la posizione degli ancoraggi
    const updateAnchorPositions = () => {
        anchorBodies.forEach(anchorBody => {
            anchorBody.position.x += (Math.random() - 0.5) * 0.3;
            anchorBody.position.y += (Math.random() - 0.5) * 0.2;
        });
    };
    ...
    const animate = () => {
        ...
        if (!paused) {
            ...
            // Aggiorna le posizioni degli ancoraggi
